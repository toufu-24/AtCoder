...
class KDNode(Node):
    ...
    def _remove(self, point):
        ...
        root.left, root.right = tmp_l if tmp_l is not root else self, (
            tmp_r if tmp_r is not root else self
        )
...
tree = create(points)
print(points)
point_to_indices = defaultdict(deque)
for idx, p in enumerate(points):
    point_to_indices[p].append(idx)
now = 0
for i in range(n):
    ret = tree.search_nn(points[now])
    node, distance = ret
    nearest_point = node.data
    nearest_index = point_to_indices[nearest_point].popleft()
    tree = tree.remove(nearest_point, node)
    print(nearest_index + 1, end=" ")
    now = nearest_index

...
Solution generate_initial_solution(vector<vector<int>> &w, vector<vector<int>> &d, XorShift &rng) {
    ...
    while (!all_boxes.empty()) {
        ...
        // 近い箱を追加していく（最大3個まで）
        while (group.boxes.size() < MAX_GROUP_SIZE && !all_boxes.empty()) {
            int best_idx = -1;
            int best_dist = 1e9;
...
Solution neighbor_move_box(Solution &sol, vector<vector<int>> &w, vector<vector<int>> &d, XorShift &rng) {
    ...
    if (new_sol.groups[g1].boxes.empty() || new_sol.groups[g2].boxes.size() >= MAX_GROUP_SIZE) {
        return sol;
    }
...
// 近傍操作：グループ内で箱の位置を再挿入（1つ取り出して別位置に挿入）
Solution neighbor_reinsert_order(Solution &sol, vector<vector<int>> &w, vector<vector<int>> &d, XorShift &rng) {
    Solution new_sol = sol;
    if (new_sol.groups.empty())
        return sol;
    int g = rng.randint(0, new_sol.groups.size() - 1);
    auto &ord = new_sol.groups[g].order;
    if (ord.size() < 3)
        return sol;
    int from = rng.randint(0, ord.size() - 1);
    int to = rng.randint(0, ord.size() - 1);
    while (from == to)
        to = rng.randint(0, ord.size() - 1);
    auto node = ord[from];
    ord.erase(ord.begin() + from);
    ord.insert(ord.begin() + to, node);
    // 耐久力チェック
    if (!check_durability(new_sol.groups[g].boxes, new_sol.groups[g].order, w, d)) {
        return sol;
    }

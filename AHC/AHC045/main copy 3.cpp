...
class RoadNetworkPlanner {
  private:
    ...
    vector<vector<int>> groupCities() {
        ...
        // 1. 最初の中心をランダムに選択
        uniform_int_distribution<int> initial_dist(0, N - 1);
        int first_centroid_index = initial_dist(rng);
        centroids[0] = {cities[first_centroid_index].expected_x, cities[first_centroid_index].expected_y};

        // 2. 残りの中心を k-means++ の方法で選択
        for (int i = 1; i < M; ++i) {
            vector<double> min_sq_dists(N, DBL_MAX);
            for (int j = 0; j < N; ++j) {
                for (int k = 0; k < i; ++k) {
                    double dx = cities[j].expected_x - centroids[k].first;
                    double dy = cities[j].expected_y - centroids[k].second;
                    min_sq_dists[j] = min(min_sq_dists[j], dx * dx + dy * dy);
                }
            }

            double sum_min_sq_dists = accumulate(min_sq_dists.begin(), min_sq_dists.end(), 0.0);
            double random_val = sum_min_sq_dists * dist(rng);
            double cumulative_sum = 0.0;
            for (int j = 0; j < N; ++j) {
                cumulative_sum += min_sq_dists[j];
                if (cumulative_sum >= random_val) {
                    centroids[i] = {cities[j].expected_x, cities[j].expected_y};
                    break;
                }
            }
        }
    ...
    vector<vector<int>> simulatedAnnealing(const vector<vector<int>> &init_groups) {
        ...
        constexpr int TIMELIMIT = 1750; // 時間制限を考慮
        mt19937 rng(1234);
        int iter = 0;
        while (true) {
            iter++;
            if (iter % 10 == 0 && chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - start).count() > TIMELIMIT)
                break;
            // ランダムに2つのグループを選択

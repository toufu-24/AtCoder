...
pair<int, int> nearest_key(pair<int, int> now, char target)
{
...
int calc_pair_cost(int a, int b)
{
    auto [ax, ay] = min_start_end[a].second;
    auto [bx, by] = min_start_end[b].first;
    return calc_move_cost(ax, ay, bx, by);
}
...
int main()
{
        ...
        for (int j = 0; j < N; j++)
        {
            for (int k = 0; k < N; k++)
            {
                if (keyboard[j][k] == target[0])
                {
                    ...
                    {
                        auto next = nearest_key(now, target[l]);
    ...
    for (int i = 0; i < ans.size(); i++)
    {
        for (int j = 0; j < ans.size(); j++)
        {
            ...
            {
                swap(ans[i], ans[j]);
                next_cost = calc_all_cost(ans);
            }
            else
            {
                next_cost = prev_cost;
                next_cost -= calc_pair_cost(ans[i - 1], ans[i]);
                next_cost -= calc_pair_cost(ans[i], ans[i + 1]);
                next_cost -= calc_pair_cost(ans[j - 1], ans[j]);
                next_cost -= calc_pair_cost(ans[j], ans[j + 1]);
                swap(ans[i], ans[j]);
                next_cost += calc_pair_cost(ans[i - 1], ans[i]);
                next_cost += calc_pair_cost(ans[i], ans[i + 1]);
                next_cost += calc_pair_cost(ans[j - 1], ans[j]);

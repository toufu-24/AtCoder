...
int32_t main() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (auto idx : single) {
            ...
            for (auto to : graph[idx]) {
                if (depth_vec[to] < h - 1 && depth_vec[to] > best_depth && !single.contains(to)) {
                    best_v = to;
                    best_depth = depth_vec[to];
                }
            }
            if (best_v != -1) {
                ans[idx] = best_v;
                visited[idx] = true;
                depth_vec[idx] = best_depth + 1;
                erase_idx.push_back(idx);
            }
        }

        for (auto idx : erase_idx) {
            single.erase(find(all(single), idx));
        }
    }
    ...
    vector<pair<int, int>> single_idx;
    for (auto s : single) {
        single_idx.push_back({a[s], s});
    }
    SORT(single_idx);

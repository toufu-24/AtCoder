#include <bits/stdc++.h>
#include <atcoder/all>
using namespace std;
using namespace atcoder;

#define all(v) v.begin(), v.end()
#define sort(v) sort(v.begin(), v.end())
#define reverse(v) reverse(v.begin(), v.end())
#define ll long long

#define BASEPOWER 50
#define BIGGERPOWER 600
#define howTimesAllow 0
#define houseX(i) house[i].first
#define houseY(i) house[i].second
#define targetAll(i) map[houseX(target)][houseY(target)]

void Xsolve(vector<vector<bool>> &map, const set<pair<int, int>> water, const vector<pair<int, int>> house);
void Ysolve(vector<vector<bool>> &map, const set<pair<int, int>> water, const vector<pair<int, int>> house);

// 土地のサイズ
int LandSize;
// 水源の数
int WaterNum;
// 家の数
int HouseNum;
// 消費体力
int Consume;
// 開通した家の数
int finish_house_num = 0;
vector<pair<int, int>> finish_house;
int target = -1;
int x, y;
bool goX = false;
bool goY = false;
int r;
bool skip = false;

void changeTarget(vector<vector<bool>> map, set<pair<int, int>> water, vector<pair<int, int>> house)
{
    target++;
    // 水源は掘らないといけないのでboolで管理
    vector<tuple<int, int, bool>> exist_water;
    bool w = false;
    vector<ll> range;
    for (int i = 0; i < LandSize; i++)
    {
        for (int j = 0; j < LandSize; j++)
        {
            if (map[i][j])
            {
                exist_water.push_back(make_tuple(i, j, false));
                range.push_back(abs(houseX(target) - i) + abs(houseY(target) - j));
            }
            else if (water.count(make_pair(i, j)))
            {
                exist_water.push_back(make_tuple(i, j, true));
                range.push_back(abs(houseX(target) - i) + abs(houseY(target) - j));
            }
        }
    }
    ll min = INT64_MAX;
    int range_size = range.size();
    for (int i = 0; i < range_size; i++)
    {
        if (range[i] < min)
        {
            min = range[i];
            tie(x, y, w) = exist_water[i];
        }
    }
    if (!w)
    {
        if (x != houseX(target))
        {
            if (x < houseX(target))
            {
                x++;
            }
            else
            {
                x--;
            }
            goX = true;
            goY = false;
        }
        else if (y != houseY(target))
        {
            if (y < houseY(target))
            {
                y++;
            }
            else
            {
                y--;
            }
            goY = true;
            goX = false;
        }
    }
}

int getr()
{
    int r;
    cin >> r;
    if (r != 0 && r != 1)
    {
        exit(0);
    }
    return r;
}

void Xsolve(vector<vector<bool>> &map, const set<pair<int, int>> water, const vector<pair<int, int>> house)
{
    int cnt = 0;
    bool difficult = false;
    while (1)
    {
        if (!skip)
            r = getr();
        skip = false;
        if (r == 1)
        {
            cnt = 0;
            difficult = false;
            map[x][y] = true;
            bool change = false;
            if (map[houseX(target)][houseY(target)])
            {
                finish_house.push_back(make_pair(x, y));
                finish_house_num++;
                changeTarget(map, water, house);
                change = true;
            }
            else if (x != houseX(target))
            {
                if (x < houseX(target))
                {
                    x++;
                }
                else
                {
                    x--;
                }
                goX = true;
                goY = false;
            }
            else if (y != houseY(target))
            {
                if (y < houseY(target))
                {
                    y++;
                }
                else
                {
                    y--;
                }
                goY = true;
                goX = false;
            }
            if (map[x][y])
            {
                skip = true;
                continue;
            }
            cout << x << " " << y << " " << BASEPOWER << endl;
        }
        else
        {
            if (difficult && goX)
            {
                while (r == 0)
                {
                    cout << x << " " << y << " " << BIGGERPOWER << endl;
                    r = getr();
                }
                skip = true;
                Ysolve(map, water, house);
            }
            else
            {
                cout << x << " " << y << " " << BIGGERPOWER << endl;
                if (cnt >= howTimesAllow)
                {
                    difficult = true;
                }
                cnt++;
            }
        }
    }
}

void Ysolve(vector<vector<bool>> &map, const set<pair<int, int>> water, const vector<pair<int, int>> house)
{
    int cnt = 0;
    bool difficult = false;
    while (1)
    {
        if (!skip)
            r = getr();
        skip = false;
        if (r == 1)
        {
            difficult = false;
            cnt = 0;
            map[x][y] = true;
            bool change = false;
            if (map[houseX(target)][houseY(target)])
            {
                finish_house.push_back(make_pair(x, y));
                finish_house_num++;
                changeTarget(map, water, house);
                change = true;
            }
            else if (y != houseY(target))
            {
                if (y < houseY(target))
                {
                    y++;
                }
                else
                {
                    y--;
                }
                goY = true;
                goX = false;
            }
            else if (x != houseX(target))
            {
                if (x < houseX(target))
                {
                    x++;
                }
                else
                {
                    x--;
                }
                goX = true;
                goY = false;
            }

            if (map[x][y])
            {
                skip = true;
                continue;
            }
            cout << x << " " << y << " " << BASEPOWER << endl;
        }
        else
        {
            if (difficult && goY)
            {
                while (r == 0)
                {
                    cout << x << " " << y << " " << BIGGERPOWER << endl;
                    r = getr();
                }
                skip = true;
                Xsolve(map, water, house);
            }
            else
            {
                cout << x << " " << y << " " << BIGGERPOWER << endl;
                if (cnt >= howTimesAllow)
                {
                    difficult = true;
                }
                cnt++;
            }
        }
    }
}

int main()
{
    cin >> LandSize >> WaterNum >> HouseNum >> Consume;
    vector<vector<bool>> map(LandSize, vector<bool>(LandSize, false));
    set<pair<int, int>> water;
    for (int i = 0; i < WaterNum; i++)
    {
        int tmp, tmp2;
        cin >> tmp >> tmp2;
        water.insert(make_pair(tmp, tmp2));
    }
    vector<pair<int, int>> house(HouseNum);
    for (int i = 0; i < HouseNum; i++)
    {
        cin >> house[i].first >> house[i].second;
    }
    sort(house);
    changeTarget(map, water, house);
    cout << x << " " << y << " " << BASEPOWER << endl;
    Xsolve(map, water, house);
}
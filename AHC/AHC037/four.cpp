...
int32_t main() {
    ...
    int cnt = 0;
    while (end - start < TIME_LIMIT / 4) {
        cnt++;
        int idx1 = uniform_int_distribution<int>(0, n / 4 - 1)(engine);
        int idx2 = uniform_int_distribution<int>(0, n / 4 - 1)(engine);
    ...
    while (end - start < TIME_LIMIT * 2 / 4) {
        cnt++;
        int idx3 = uniform_int_distribution<int>(n / 4, n / 2 - 1)(engine);
        int idx4 = uniform_int_distribution<int>(n / 4, n / 2 - 1)(engine);
    ...
    while (end - start < TIME_LIMIT * 3 / 4) {
        cnt++;
        int idx1 = uniform_int_distribution<int>(n / 2, 3 * n / 4 - 1)(engine);
        int idx2 = uniform_int_distribution<int>(n / 2, 3 * n / 4 - 1)(engine);
        swap(a_b[idx1], a_b[idx2]);
        auto [new_cost, new_ans] = execute(a_b);
        if (new_cost < cost) {
            cost = new_cost;
    ...
    while (end - start < TIME_LIMIT) {
        cnt++;
        int idx3 = uniform_int_distribution<int>(3 * n / 4, n - 1)(engine);
        int idx4 = uniform_int_distribution<int>(3 * n / 4, n - 1)(engine);
        swap(a_b[idx3], a_b[idx4]);
        auto [new_cost, new_ans] = execute(a_b);
        if (new_cost < cost) {
            cost = new_cost;
